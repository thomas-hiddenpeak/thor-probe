...
void TimelineLogger::log_stats(const AudioPipelineStats& st,
    ...
    std::string wle_name_escaped = communis::json_escape(
        st.wlecapa_name ? st.wlecapa_name : "");
    ...
    if (st.wlecapa_active && n < (int)sizeof(buf)) {
        ...
        if (change_valid && n < (int)sizeof(buf)) {
            int w = snprintf(buf + n, sizeof(buf) - n,
    ...
    // Always ensure proper closing and null termination
    if (n < (int)sizeof(buf) - 2) {
        buf[n++] = '}';
        buf[n++] = '\n';
        buf[n] = '\0';
    } else {
        int end = (int)sizeof(buf) - 2;
        buf[end++] = '}';
        buf[end++] = '\n';
        buf[end] = '\0';
    }
...
void TimelineLogger::log_asr(const char* text, float stream_start, float stream_end,
    ...
    std::string text_escaped = communis::json_escape(text ? text : "");
    std::string trigger_escaped = communis::json_escape(trigger ? trigger : "");
    std::string spk_name_escaped = communis::json_escape(spk_name ? spk_name : "");
    std::string spk_src_escaped = communis::json_escape(spk_src ? spk_src : "");
    ...
    int n = snprintf(buf, sizeof(buf),
        ...
        "\n",
        (unsigned long)t0_ns,
        stream_start, stream_end, text_escaped.c_str(),
        trigger_escaped.c_str(), latency_ms, audio_sec,
        spk_id, spk_name_escaped.c_str(), spk_sim, spk_conf,
        spk_src_escaped.c_str());
...
void TimelineLogger::log_drop(uint64_t t1_from, uint64_t t1_to,
    ...
    std::string reason_escaped = communis::json_escape(reason ? reason : "unknown");
